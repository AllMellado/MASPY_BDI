    ...
    def _check(self, condition: Condition, args: tuple = tuple()) -> tuple | None:
        ...
        if cnd_type == "~":
            if isinstance(condition.left_value, Belief | Goal) and self.get(condition.left_value, ck_src=False) is None:
                return args
            elif isinstance(condition.left_value, Condition) and self._check(condition.left_value, args) is None:
                return args
            else:
                return None
        ...
        self.print(f'Checking: {v0_data}({v0_args}) {v1_data}({v1_args}) {args} {ret_bool}')  
        if not ret_bool:
            return None
