...
class Initiator(Agent):
    ...
    def start(self, src):
        ...
        self.send(broadcast, Belief("request_offer", randint(15,30)))
    ...
    def offer(self, src, offered_value, half_value):
        ...
        if offered_value < half_value:
            ...
            self.send(src, Belief("offer_refused", offered_value))
        else:
            self.print(f"{src}'s offer ({offered_value}) accepted")
            self.send(src, Belief("offer_accepted", offered_value))
...
class Participant(Agent):
    ...
    def request_offer(self, src, value, max_value):
        ...
        if value > 1.5 * max_value:
            ...
            self.send(src, Goal("offer", offered_value))
