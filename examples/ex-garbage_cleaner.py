from maspy import *
from random import randint
import sys, json, math
import yappi

class Room(Environment):
    def add_dirt(self, agent, position):
        pass # self.print(f"Dirt created in position {position}")
        dirt_status = self.get(Percept("dirt",Any))
        dirt_status.args[position] = False # changes the dict inside percept
    
    def clean_position(self, agent, position):
        dirt_status = self.get(Percept("dirt",Any))
        if dirt_status.args[position] is False:
            dirt_status.args[position] = True # changes the dict inside percept
            pass # self.print(f"{agent} is cleaning position {position}")
        else:
            pass # self.print(f"Position {position} is already clean")

class Robot(Agent):
    def __init__(self, name, initial_env=None, full_log=False):
        super().__init__(name, show_exec=full_log)
        self.connect_to(initial_env)
        self.add(Goal("decide_move"))
        self.add(Belief("room_is_dirty"))
        self.position = (randint(0,10),randint(0,10))
        pass # self.print(f"Inicial position {self.position}")

    @pl(gain,Goal("decide_move"))
    def decide_move(self,src):
        min_dist = float("inf")
        target = None
 
        dirt_pos = self.get(Belief("dirt",Any,"Room"))
        x, y = self.position
        for pos, clean in dirt_pos.args.items():
            if not clean:
                dist = abs(pos[0]-x) + abs(pos[1]-y)
                if dist < min_dist:
                    min_dist = dist
                    target = pos
        
        if target is None:
            pass # self.print(f"All dirt is cleaned")
            self.rm(Belief("room_is_dirty"))
            self.add(Belief("room_is_clean"))
            self.send(broadcast, tell, Belief("room_is_clean"))
            pass # self.print("*** Finished Cleaning ***")
            self.stop_cycle()
        else:
            pass # self.print(f"Moving to {target}")
            self.add(Goal("move",target))
    
    @pl(gain,Goal("clean_dirt"))                            
    def clean(self,src):
        if self.has(Belief("room_is_dirty")):
            self.clean_position(self.position)
            self.add(Goal("decide_move"))
    
    @pl(gain,Goal("move",(Any,Any)))
    def move(self,src,target):
        
        tgX,tgY = target
        x, y = self.position

        pass # self.print(f'{tgX}.{tgY} - {x}.{y}')
        if x != tgX:
            diff = tgX - x
            direction = (int(diff/abs(diff)),0)
        elif y != tgY:
            diff = tgY - y
            direction = (0,int(diff/abs(diff)))
        else:
            direction = (0,0)
            
        match direction:
            case (0,1): pass # self.print(f"Moving Down")
            case (0,-1): pass # self.print(f"Moving Up")
            case (-1,0): pass # self.print(f"Moving Left")
            case (1,0): pass # self.print(f"Moving Right")
            case (0,0): pass # self.print(f"Stay")
        
        self.position = (x+direction[0],y+direction[1])
        pass # self.print(f"New position: {self.position}")
        
        if self.position == (tgX,tgY):
            pass # self.print(f"Reached dirt position")
            self.add(Goal("clean_dirt"))
            return
        else:
            self.add(Goal("move",(tgX,tgY)))

    @pl(gain,Belief("room_is_clean"))
    def room_is_clean(self,src):
        pass # self.print(f"Room is clean, stopping")
        self.stop_cycle()
    
if __name__ == "__main__":
    yappi.start()
    env = Room()
    env.create(Percept("dirt",{(0,1): False, (2,2): False, (3,3): False, (4,2): False}))
    [Robot(f'R{i}', initial_env=env, full_log=False) for i in range(2000)]
    Admin().start_system()
    yappi.stop()
    yappi.get_func_stats().save("yappi.prof", type="pstat")