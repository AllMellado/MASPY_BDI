...
end_flag = 0
success_flag = 0
fail: dict[str, int] = {"reject":0,"occupied":0,"lost":0}
...
class Parking(Environment):
    ...
    def reserve_spot(self, agt, spot_id):
        spot = self.get(Percept("spot",(spot_id,"free")))
        if spot:
            self.change(spot,(spot_id,"reserved"))
            return True
        else:
            return False
...
class Manager(Agent):
    ...
    def offer_response(self,src,offer_answer,min_price):
        ...
        pass # self.print(f"{self.cycle_counter} Received answer[{answer}] from {src}")
        match answer:
            case "reject":
                pass # self.print(f"{self.cycle_counter} Given price[{price}] rejected by {src}")
                pass
            case "accept":
                pass # self.print(f"{self.cycle_counter} Price accepted[{price}] by {src}. Choosing spot.")
                self.add(Goal("SendSpot",src),True)
            case "offer":
                if price < min_price:
                    counter_offer = (min_price+price)/1.8
                    counter_offer = round(counter_offer,2)
                    pass # self.print(f"{self.cycle_counter} Price offered[{price}] from {src} too low. Counter-offer[{counter_offer}]")
                    self.send(src,achieve,Goal("checkPrice",counter_offer),"Parking")
                else:
                    pass # self.print(f"{self.cycle_counter} Offered price from {src} accepted[{price}]. Choosing spot.")
                    self.add(Goal("SendSpot",src),True)
...
def main(num_agents: int = 10):
    ...
    Admin().report = True
    Admin().start_system()
    print(Admin().report_buffer)
    print(f'success: {success_flag}, failures: {fail}')
...
if __name__ == "__main__":
    ...
    # stats.dump_stats(filename="ex_parking.prof")

    # main_buffer = ""
    # for i in [1000,2000,3000,4000,4000,5000]:
    #     for _ in range(2):
    #         buffer = main(i)
    #         print(buffer)
    #         main_buffer += buffer + "--\n"
