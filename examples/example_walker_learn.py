...
class MapVisualizer:
    def __init__(self, env) -> None:
        pygame.init()
        self.env = env
        width = env.map_size[0]*CELL_SIZE + SIDEBAR_WIDTH
        height = env.map_size[1]*CELL_SIZE
    ...
    def _create_buttons(self):
        buttons = []
        x = self.env.map_size[0]*CELL_SIZE + 20
        y = 40
        spacing = 60
    ...
    def draw_sidebar(self):
        sidebar_rect = pygame.Rect(self.env.map_size[0]*CELL_SIZE, 0, SIDEBAR_WIDTH, self.env.map_size[1]*CELL_SIZE)
    ...
    def draw_map(self):
        # grid
        for x in range(0, self.env.map_size[0]*CELL_SIZE, CELL_SIZE):
            pygame.draw.line(self.screen, GRID_COLOR, (x, 0), (x, self.env.map_size[1]*CELL_SIZE))
        for y in range(0, self.env.map_size[1]*CELL_SIZE, CELL_SIZE):
            pygame.draw.line(self.screen, GRID_COLOR, (0, y), (self.env.map_size[0]*CELL_SIZE, y))

        # draw target
        target = self.env.get(Percept("target", (Any, Any)))
        if target:
            tx, ty = target.args
            cx, cy = tx * CELL_SIZE, ty * CELL_SIZE
            pygame.draw.line(self.screen, TARGET_COLOR,
                            (cx + 4, cy + 4),
                            (cx + CELL_SIZE - 4, cy + CELL_SIZE - 4), 3)
            pygame.draw.line(self.screen, TARGET_COLOR,
                            (cx + CELL_SIZE - 4, cy + 4),
                            (cx + 4, cy + CELL_SIZE - 4), 3)
...
class MenuScreen:
    def __init__(self):
        ...
        self.clock = pygame.time.Clock()
        self.inputs = [
            TextInput((200, 100, 150, 35), "Map Width:", "10"),
            TextInput((200, 160, 150, 35), "Map Height:", "10"),
            TextInput((200, 220, 150, 35), "Agents:", "3")
        ]
