...
if __name__ == "__main__":
    forth = math.ceil(int(sys.argv[1])/4)
    with open(r"T:\Downloads\measuring\duration.json","w") as f:
        json.dump({"running": "contract-net", "num_agents": sys.argv[1]}, f)
    num_initiators = forth
    num_participants = forth*3
    [Initiator() for _ in range(num_initiators)]
    [Participant() for _ in range(num_participants)] 
    Admin().start_system()
    with open(r"T:\Downloads\measuring\duration.json","w") as f:
